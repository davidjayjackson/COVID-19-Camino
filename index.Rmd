...
date: "2021-01-07 01:50PM PM/EST"
output: 
  html_document: 
    fig_width: 9
    fig_height: 5
...
ggplot(A) + # geom_line(aes(x=date,y=Rate)) +
  ...
  labs(title="Spain COVID19 Mortality Rate ",x="Date date",y="Mortality Rate") +
...
```{r}
daily_new_cases <-ggplot(US) +  # geom_line(aes(x=date,y=new_cases,col="Daily new_cases")) +
  labs(title="COVID-19 new_cases by Date") +
    geom_smooth(aes(x=date,y=new_cases,col="Loess"),span=0.15) +
   scale_alpha_date()

daily_new_deaths <-ggplot(US) + # geom_line(aes(x=date,y=new_deaths,col="Daily new_deaths")) +
  labs(title="COVID-19 new_deaths by Date") + ylim(0,1000) +
